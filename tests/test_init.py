...
from custom_components.onkyo import async_setup_entry, async_unload_entry, async_migrate_entry
...
async def test_setup_entry_success(hass, mock_entry):
    ...
    with patch("custom_components.onkyo.eISCP") as mock_eiscp, \
...
async def test_setup_entry_os_error(hass, mock_entry):
    ...
    mock_entry.add_to_hass(hass)
    with patch("custom_components.onkyo.eISCP") as mock_eiscp, \
         patch("custom_components.onkyo._test_connection", side_effect=OSError("Network unreachable")), \
         patch("homeassistant.config_entries.ConfigEntries.async_forward_entry_setups") as mock_forward:
...
async def test_setup_entry_unexpected_error(hass, mock_entry):
    ...
    mock_entry.add_to_hass(hass)
    with patch("custom_components.onkyo.eISCP") as mock_eiscp, \
         patch("custom_components.onkyo._test_connection", side_effect=Exception("Boom")):
...
async def test_unload_entry(hass, mock_entry):
    ...
    # Then unload
    with patch("custom_components.onkyo.connection.OnkyoConnectionManager.async_close") as mock_close, \
