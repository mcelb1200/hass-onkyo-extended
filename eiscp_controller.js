  ...
  function EISCPControllerNode(config) {
    ...
    this.initializeEISCPConnection = function (handler) {
      try {
        if (node.eiscpjsconn) {
          node.log(
            "already configured to EISCP device at " +
              config.host +
              ":" +
              config.port +
              " model[" +
        ...
        node.log(
          "configuring to EISCP device at " +
            config.host +
            ":" +
            config.port +
            " model[" +
        ...
        node.log(
          "EISCP: successfully connected to " +
            config.host +
            ":" +
            config.port +
            " model[" +
        ...
        if (handler && typeof handler === "function") handler(node.eiscpjsconn);
        return node.eiscpjsconn;
      } catch (err) {
    ...
    this.on("close", function () {
      node.log(
        "disconnecting from eiscpjs server at " +
          config.host +
          ":" +
          config.port +
          " model[" +
